...
const isNewsArticle = (article: BlogArticle) => article.contentType
  ...
  : ["construction news", "market update", "industry", "regulation", "project update", "market news"].some((category) => article.category.toLowerCase().includes(category));
...
async function relatedArticles(article: BlogArticle) {
  ...
  return source
    ...
    .sort((a, b) => {
      const sameTypeA = isNewsArticle(a) === isNewsArticle(article) ? 1 : 0;
      const sameTypeB = isNewsArticle(b) === isNewsArticle(article) ? 1 : 0;
      if (sameTypeA !== sameTypeB) return sameTypeB - sameTypeA;
      ...
      <article>
        <header className="relative overflow-hidden bg-[#071E33] text-white">
          ...
          <div className="relative mx-auto max-w-7xl px-5 py-12 md:px-8 md:py-16">
            ...
            <div className="mt-9 max-w-5xl">
              ...
              <div className="mt-7 flex flex-wrap items-center gap-x-5 gap-y-3 text-xs text-white/52"><span className="font-bold text-white/75">By {author}</span><span className="flex items-center gap-2"><CalendarDays className="h-4 w-4" />{new Date(article.publishedAt).toLocaleDateString("en-NG", { day: "numeric", month: "long", year: "numeric" })}</span><span>{article.readTime}</span></div>
            </div>
          </div>
        </header>

        {article.imageUrl ? <div className="mx-auto max-w-7xl px-5 pt-8 md:px-8 md:pt-10"><div className="aspect-[16/7] overflow-hidden bg-[#EEF2F6]"><img src={article.imageUrl} alt={article.imageAlt || article.title} className="h-full w-full object-cover" /></div></div> : null}
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 md:px-8 md:py-16 lg:grid-cols-[minmax(0,1fr)_300px]">
          ...
          <aside className="space-y-6 lg:sticky lg:top-28 lg:self-start">
            ...
            {related.length ? <section className="border border-[#0D3B66]/10 bg-white p-6"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#C8A45D]">Continue reading</p><div className="mt-3 divide-y divide-[#0D3B66]/10">{related.map((item) => <Link key={item.slug} href={`/blog/${item.slug}`} className="group grid grid-cols-[64px_1fr] gap-3 py-4"><div className="h-14 overflow-hidden bg-[#EEF2F6]">{item.imageUrl ? <img src={item.imageUrl} alt="" className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center"><ImageIcon className="h-4 w-4 text-[#9AA8B6]" /></div>}</div><div><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#3A4653]/50">{item.category}</p><h3 className="mt-1 text-sm font-semibold leading-5 text-[#071E33] transition group-hover:text-[#0D3B66]">{item.title}</h3><span className="mt-2 inline-flex items-center gap-1 text-[10px] font-bold text-[#0D3B66]">Read <ArrowRight className="h-3 w-3" /></span></div></Link>)}</div></section> : null}
          </aside>
        </div>
      </article>
    </main>
