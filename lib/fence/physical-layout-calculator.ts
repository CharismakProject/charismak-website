...
const validatePanelComposition = (
  ...
  if (
    ...
    throw new Error(
      "Block-wall height cannot be negative."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Upper-infill height cannot be negative."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "An upper infill requires a positive height."
    );
  }
...
export function resolveFenceColumnSpecification(
  input: {
  ...
  if (column.roles.includes("gate-post")) {
    const relatedGates = column.relatedGateIds.map(
      (gateId) => {
...
export function calculateFenceSectionPhysicalLayout(
  input: {
  ...
  if (
    ...
    throw new Error(
      "Column-body height must be greater than zero."
    );
  }
  ...
  const columnLayout =
    calculateFenceSectionColumnLayout(section);
  ...
  for (const column of columnLayout.columns) {
    const specification =
      resolveFenceColumnSpecification({
        column,
        section,
        specifications,
      });
    ...
    resolvedColumnMap.set(column.id, {
      ...
      constructionSystem:
        specification.constructionSystem,
      widthAlongFenceM:
...
let calculatedUpperInfillAreaM2 = 0;
  ...
  for (const wallSegment of segmentation.wallSegments) {
    ...
    const resolvedSegmentColumns = segmentColumns.map(
      (column) => {
        const resolvedColumn = resolvedColumnMap.get(
          column.id
        );

        if (!resolvedColumn) {
          throw new Error(
            `Column "${column.id}" could not be resolved.`
          );
        }
    ...
    const totalSegmentColumnWidthM =
      resolvedSegmentColumns.reduce(
        (total, column) =>
          total + column.widthAlongFenceM,
        0
      );

    const totalClearPanelLengthM =
      wallSegment.grossLengthM -
      totalSegmentColumnWidthM;

    if (
      totalClearPanelLengthM <
      -PHYSICAL_LAYOUT_TOLERANCE
    ) {
      throw new Error(
    ...
    const panelGapCount =
      resolvedSegmentColumns.length - 1;
    ...
    for (
      ...
      const column =
        resolvedSegmentColumns[columnIndex];
      ...
      const occupiedEndM =
        occupiedStartM + column.widthAlongFenceM;
      ...
      const positionedColumn: ResolvedFenceColumnPlacement =
        {
          ...
          occupiedStartM:
            roundPhysicalQuantity(occupiedStartM),
          occupiedEndM:
            roundPhysicalQuantity(occupiedEndM),
        };

      resolvedColumnMap.set(
        positionedColumn.id,
        positionedColumn
      );
...
calculatedUpperInfillAreaM2 += upperInfillAreaM2;
          ...
          panels.push({
            ...
            panelNumber,
            startM:
              roundPhysicalQuantity(panelStartM),
            endM: roundPhysicalQuantity(panelEndM),
            clearLengthM:
              roundPhysicalQuantity(clearPanelLengthM),
            leftColumnId: column.id,
            rightColumnId:
              resolvedSegmentColumns[columnIndex + 1]
                .id,
            composition,
            blockworkAreaM2:
              roundPhysicalQuantity(blockworkAreaM2),
            upperInfillAreaM2:
              roundPhysicalQuantity(
                upperInfillAreaM2
              ),
          });
        }
  ...
  const columns = Array.from(
    ...
    (firstColumn, secondColumn) =>
      firstColumn.occupiedStartM -
      secondColumn.occupiedStartM
  );
...
const totalClearBlockPanelLengthM =
  calculatedClearPanelLengthM;

const totalBlockworkAreaM2 =
  calculatedBlockworkAreaM2;

const totalUpperInfillAreaM2 =
  calculatedUpperInfillAreaM2;
  ...
  if (
    Math.abs(
      physicalLengthCheckM - section.grossLengthM
    ) >
    PHYSICAL_LAYOUT_TOLERANCE * 10
  ) {
    throw new Error(
  ...
  return {
    ...
    grossSectionLengthM: roundPhysicalQuantity(
    ...
    totalBlockworkAreaM2: roundPhysicalQuantity(
      totalBlockworkAreaM2
    ),
