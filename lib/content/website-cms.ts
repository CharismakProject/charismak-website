...
import { people, type Person, type Project } from "@/app/site-data";
...
export type ManagedWebsiteProject = Project & {
  published: boolean;
  displayOrder: number;
};

export type ManagedWebsitePerson = Person & {
  id: string;
  published: boolean;
  displayOrder: number;
};

export type ManagedWebsiteContent = {
  contentKey: string;
  section: string;
  label: string;
  value: unknown;
...
const rowToProject = (row: Record<string, unknown>): ManagedWebsiteProject => ({
  ...
  heroTitle: row.hero_title ? String(row.hero_title) : undefined,
  publicCategory: String(row.public_category ?? "Charismak Project") as Project["publicCategory"],
  engagementTag: String(row.engagement_tag ?? "Direct Contract") as Project["engagementTag"],
  role: String(row.role ?? "Main Contractor"),
  organisation: String(row.organisation ?? "Charismak Project Nigeria Limited"),
  location: String(row.location ?? "Abuja, Nigeria"),
  status: String(row.project_status ?? "Ongoing"),
  client: row.client ? String(row.client) : undefined,
  summary: String(row.summary ?? ""),
  attribution: String(row.attribution ?? ""),
  cover: String(row.cover_url ?? ""),
  heroImages: strings(row.hero_images),
  images: strings(row.gallery_images),
  videos: strings(row.videos),
  services: strings(row.services),
  featured: Boolean(row.featured),
  showOnProjectsPage: Boolean(row.published),
...
const rowToPerson = (row: Record<string, unknown>): ManagedWebsitePerson => ({
  ...
  group: String(row.group_name ?? "Supporting Team") as Person["group"],
  category: String(row.category ?? "Project Delivery"),
  bio: String(row.bio ?? ""),
  published: Boolean(row.published),
  displayOrder: Number(row.display_order ?? 100),
});
...
export async function loadWebsiteContent(section?: string): Promise<ManagedWebsiteContent[]> {
  try {
    const client = publicClient();
    let query = client.from("website_content").select("*").eq("published", true);
