...
export type NewMarketplaceProfile = Pick<
  MarketplaceProfile,
  "type" | "businessName" | "category" | "location" | "serviceArea" | "phone" | "email" | "description"
>;
...
export const starterMarketplaceProfiles: MarketplaceProfile[] = [
  {
    id: "seed-supplier-abuja-1",
    type: "supplier",
    businessName: "Example Aggregate Supplier",
    category: "Cement, blocks & aggregates",
    categories: ["Cement, blocks & aggregates"],
    location: "Abuja",
    serviceArea: "FCT",
    phone: "",
    email: "",
    description: "",
    products: ["Cement · bag", "Sharp sand · 10 m³ truck", "Granite · 30-tonne truck", "Blocks · 9-inch piece"],
    offers: [],
    rating: 0,
    reviewCount: 0,
    verified: false,
  ...
  {
    id: "seed-supplier-lagos-1",
    type: "supplier",
    businessName: "Example Steel & Mesh Supplier",
    category: "Reinforcement & BRC mesh",
    categories: ["Reinforcement & BRC mesh"],
    location: "Lagos",
    serviceArea: "Lagos State",
    phone: "",
    email: "",
    description: "",
    products: ["Y12 rebar · 12 m length", "Y16 rebar · 12 m length", "A142 BRC · 2.4 × 4.8 m sheet", "Binding wire · 25 kg roll"],
    offers: [],
    rating: 0,
    reviewCount: 0,
    verified: false,
  ...
  {
    id: "seed-artisan-abuja-1",
    type: "artisan",
    businessName: "Example Formwork Team",
    ...
    email: "",
    description: "",
    products: ["Slab formwork · m² labour", "Column formwork · m² labour", "Carpentry crew · day"],
    offers: [],
    rating: 0,
    reviewCount: 0,
    verified: false,
  ...
  {
    id: "seed-artisan-ph-1",
    type: "artisan",
    businessName: "Example Plumbing Team",
    category: "Plumbing & mechanical",
    categories: ["Plumbing & mechanical"],
    location: "Port Harcourt",
    serviceArea: "Port Harcourt metropolitan area",
    phone: "",
    email: "",
    description: "",
    products: ["First-fix plumbing · point", "Sanitary fitting · item", "Call-out inspection · visit"],
    offers: [],
    rating: 0,
    reviewCount: 0,
    verified: false,
...
const toSupplierDirectoryProfile = (row: Record<string, unknown>): MarketplaceProfile => {
  ...
  const offers: MarketplaceOffer[] = rawOffers
    .map((offer) => {
...
export async function loadMarketplaceProfiles(): Promise<MarketplaceProfile[]> {
  ...
  if (!client) return starterMarketplaceProfiles;

  const { data, error } = await client.functions.invoke("public-supplier-directory", { body: {} });
  if (error || !Array.isArray(data?.profiles)) return starterMarketplaceProfiles;

  const remote = (data.profiles as Record<string, unknown>[]).map(toSupplierDirectoryProfile);
  return remote.length ? remote : starterMarketplaceProfiles;
}

export async function submitMarketplaceProfile(
  input: NewMarketplaceProfile,
): Promise<{ profile: MarketplaceProfile; published: boolean }> {
  const base: MarketplaceProfile = {
    ...input,
    id: `local-market-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
