        ...
        <div className="grid gap-6 p-5 md:p-8 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="space-y-5">
            ...
            <section className="rounded-2xl border border-[#DCE4EC] bg-white p-4 md:p-5">
              ...
              {profile.offers.length ? <div className="mt-4 space-y-3">{profile.offers.map((offer) => <div key={offer.id} className="rounded-xl border border-[#E1E8EF] bg-[#F8FAFC] p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><strong className="block text-sm text-[#081B36]">{offer.productName}</strong><span className="mt-1 block text-xs text-[#617286]">Quoted unit: {offer.quotedUnit}</span></div><strong className="text-lg text-[#A82B05]">{money(offer.price)}</strong></div><div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 border-t border-[#E1E8EF] pt-3 text-[10px] text-[#7A8B9E]"><span>Valid from {formatDate(offer.validFrom)}</span><span>Valid until {formatDate(offer.validUntil)}</span></div></div>)}</div> : <div className="mt-4 rounded-xl border border-dashed border-[#B8C7D6] bg-[#F8FAFC] p-5 text-center"><PackageSearch className="mx-auto h-6 w-6 text-[#7A8B9E]" /><p className="mt-2 text-sm font-bold text-[#081B36]">No approved product price published yet</p><p className="mt-1 text-xs leading-5 text-[#617286]">This supplier profile is active, but its product / service submissions have not yet been approved for public display.</p></div>}
