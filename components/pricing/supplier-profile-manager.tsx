...
export default function SupplierProfileManager({ profile, onDone, onSignOut }: { profile: SupplierProfile; onDone: (profile: SupplierProfile) => void; onSignOut: () => void }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "We could not save the profile.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <section className="rounded-[1.5rem] border border-[#DCE4EC] bg-white p-4 shadow-[0_10px_35px_rgba(7,30,51,.06)] sm:rounded-[2rem] sm:p-7">
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row">
        ...
        <button onClick={signOut} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-[#E5B8AE] px-5 text-sm font-black text-[#A82B05]"><LogOut className="h-4 w-4" />Sign out / switch supplier</button>
      </div>
    </section>
  );
}
