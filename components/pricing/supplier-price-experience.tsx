...
import SupplierReviewRequests from "@/components/pricing/supplier-review-requests";
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-[#DCE4EC] bg-white p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-[#617286]">
              {showBulk ? "Add or update several products together." : "Submit a price without creating a supplier account."}
            </p>
          </div>
          <button
            type="button"
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col gap-3 rounded-2xl border border-[#DCE4EC] bg-white p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
        ...
        <button type="button" onClick={() => setShowSingle(false)} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#CBD7E2] bg-white px-4 text-xs font-black text-[#071E33] sm:w-auto">
...
function SupplierPricingStart({
  ...
  return (
    <div className="space-y-5 sm:space-y-7">
      {newlyCreated ? (
        <section className="rounded-[1.5rem] border border-[#BFE2CD] bg-[#F0FAF4] p-5 sm:p-6">
          <div className="flex items-start gap-3">
            <span className="mt-0.5 grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white text-[#197447] shadow-sm"><CheckCircle2 className="h-5 w-5" /></span>
            <div>
      ...
      <SupplierReviewRequests profile={profile} onUpdateMyself={onSingle} />
      ...
      <section className="overflow-hidden rounded-[1.5rem] border border-[#DCE4EC] bg-white shadow-[0_18px_55px_rgba(7,30,51,0.08)] sm:rounded-[2rem]">
        <div className="border-b border-[#E5EBF0] bg-[#071E33] p-5 text-white sm:p-7">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.14em] text-[#F2B544]"><UserRound className="h-4 w-4" /> {profile.supplierCode}</span>
              <h2 className="mt-3 text-2xl font-black sm:text-3xl">{profile.businessName}</h2>
              <p className="mt-2 inline-flex items-center gap-2 text-xs text-white/65"><MapPin className="h-3.5 w-3.5 text-[#F2B544]" /> {profile.location}</p>
            </div>
            <button type="button" onClick={onManage} className="min-h-11 rounded-xl border border-white/20 bg-white/10 px-4 text-xs font-black text-white">Edit profile</button>
          </div>
        </div>
        ...
        <div className="p-5 sm:p-7">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[#617286]">Bulk pricing is the main option for suppliers with several products. Use single pricing when you only need to add or change one item.</p>
          </div>

          <div className="mt-6 grid gap-4 md:grid-cols-[1.25fr_0.75fr]">
            <button type="button" onClick={onBulk} className="group rounded-[1.4rem] border-2 border-[#0D3B66] bg-[#F2F7FC] p-5 text-left transition hover:-translate-y-0.5 hover:shadow-[0_16px_40px_rgba(13,59,102,0.12)] sm:p-6">
              <div className="flex items-start justify-between gap-4"><span className="grid h-12 w-12 place-items-center rounded-xl bg-[#0D3B66] text-white"><Layers3 className="h-6 w-6" /></span><span className="rounded-full bg-[#F2B544] px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.12em] text-[#071E33]">Recommended</span></div>
            ...
            <button type="button" onClick={onSingle} className="group rounded-[1.4rem] border border-[#D8E1E8] bg-white p-5 text-left transition hover:border-[#C8A45D] sm:p-6">
              <span className="grid h-12 w-12 place-items-center rounded-xl bg-[#F3F6F9] text-[#0D3B66]"><PackagePlus className="h-6 w-6" /></span>
          ...
          {newlyCreated ? (
            <div className="mt-6 border-t border-[#E5EBF0] pt-5">
              <a href={whatsappHref} target="_blank" rel="noreferrer" onClick={onSavedWhatsApp} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#BFE2CD] bg-[#F0FAF4] px-4 text-xs font-black text-[#197447]"><MessageCircle className="h-4 w-4" /> Save supplier access on WhatsApp</a>
            </div>
          ) : null}
        </div>
      </section>
