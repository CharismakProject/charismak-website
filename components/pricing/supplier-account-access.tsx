  ...
  return (
    <div className="space-y-5 sm:space-y-7">
      <section className="overflow-hidden rounded-[1.5rem] bg-[#071E33] p-5 text-white shadow-[0_24px_65px_rgba(7,30,51,0.18)] sm:rounded-[2rem] sm:p-8">
        ...
        <h1 className="mt-4 max-w-3xl text-3xl font-black leading-tight sm:text-5xl">Join the Charismak supplier network.</h1>
        <p className="mt-4 max-w-2xl text-sm leading-7 text-white/70">List your business, show what you supply and keep your current prices available to buyers.</p>
      </section>
      ...
      <section className="rounded-[1.5rem] border border-[#DCE4EC] bg-white p-4 shadow-[0_10px_35px_rgba(7,30,51,0.05)] sm:rounded-[2rem] sm:p-7">
        ...
        {mode === "signin" ? <div className="mt-4 flex flex-col gap-2 rounded-xl border border-[#CFE4D7] bg-[#F3FBF6] p-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-[#526579]"><strong className="text-[#071E33]">Forgot your PIN?</strong> We’ll verify the registered contact before resetting it.</p><a href={supportWhatsApp(businessName, phone)} target="_blank" rel="noreferrer" className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-[#197447] px-4 text-xs font-black text-white"><MessageCircle className="h-4 w-4" />WhatsApp us</a></div> : null}
        ...
        <div className="mt-6 border-t border-[#E4E9EE] pt-5"><button type="button" onClick={onGuest} className="text-xs font-bold text-[#0D3B66] underline underline-offset-4">Submit prices without creating a profile</button></div>
      </section>
    </div>
  );
}
