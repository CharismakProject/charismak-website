...
import { createNewBill, saveBill } from "@/lib/billing/store";
...
export default function GuidedEstimator({
  ...
  const [scope, setScope] = useState<ProjectScope>({
    ...
    steelWeightTonnes: existing?.steelWeightTonnes ?? null,
  ...
  const isSteel = project.projectType === "structural-steel";
  ...
  const steelWeight = scope.steelWeightTonnes ?? 0;
  ...
  const buildScope = (): ProjectScope => ({
    ...
    floorAreaM2: isSteel ? scope.floorAreaM2 : footprint || scope.floorAreaM2,
    ...
    source: mode,
    confidence: isSteel ? (steelWeight > 0 ? "detailed" : "rough") : footprint > 0 ? "detailed" : "rough",
  });
  ...
  const saveScope = () => {
    ...
    setScope(savedScope);
    setMessage("Project information saved and syncing to your account.");
    return savedScope;
  };
  ...
  const createPreliminaryBill = () => {
    ...
    if (latest.basisQuantity <= 0) {
      setMessage(isSteel ? "Enter an approximate structural steel tonnage before creating a preliminary cost plan." : "Enter a valid project size before creating a preliminary cost plan.");
      return;
    }
    let bill = createNewBill({
      title: `${project.name} — Preliminary Cost Plan`,
  ...
  const useAsBudget = () => {
    ...
    const latest = calculateGuidedEstimate(project.projectType, savedScope);
    if (latest.basisQuantity <= 0) {
      setMessage(isSteel ? "Enter an approximate structural steel tonnage before using this as a project budget." : "Enter a valid project size before using this as a project budget.");
      return;
    }
  ...
  return (
    <div className="space-y-5">
      ...
      {step === 1 ? <section className="rounded-2xl border border-[#DCE4EC] bg-white p-4 md:p-6">
        <div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#EAF2FF] text-[#175FC4]"><Landmark className="h-5 w-5" /></span><div><h2 className="text-lg font-bold text-[#081B36]">{isSteel ? "Structural steel quantity" : "Land and project size"}</h2><p className="mt-1 text-xs leading-5 text-[#617286]">{isSteel ? "Use the structural engineer's take-off or your best current steel tonnage. This is a planning basis, not a substitute for a steel schedule." : "Use metres. If you only know the floor area, enter it directly."}</p></div></div>
        {isSteel ? <div className="mt-5 max-w-xl"><label className="block rounded-xl border border-[#DCE4EC] bg-[#F8FAFC] p-4 text-xs font-semibold text-[#526579]">Approximate structural steel weight (tonnes)<input type="number" min="0" step="0.1" value={scope.steelWeightTonnes ?? ""} onChange={(event) => update("steelWeightTonnes", event.target.value ? numberValue(event.target.value) : null)} placeholder="e.g. 18.5" className="mt-2 w-full rounded-xl border border-[#CAD5E0] bg-white px-3 py-3 text-base text-[#081B36]" /><span className="mt-2 block font-normal leading-5 text-[#617286]">Include the approximate fabricated structural steel quantity only. Roofing sheets, reinforcement bars and aluminium are priced separately.</span></label></div> : <><div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <div className="mt-5 flex flex-wrap gap-3"><button type="button" disabled={isSteel && steelWeight <= 0} onClick={() => setStep(2)} className="inline-flex items-center gap-2 rounded-xl bg-[#081B36] px-5 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">Continue <ArrowRight className="h-4 w-4" /></button>{isSteel ? <span className="inline-flex items-center gap-2 rounded-xl bg-[#F4F7FA] px-4 py-3 text-xs text-[#617286]"><Info className="h-4 w-4" />{steelWeight > 0 ? `${steelWeight.toFixed(1)} tonnes entered` : "Enter tonnage to continue"}</span> : <span className="inline-flex items-center gap-2 rounded-xl bg-[#F4F7FA] px-4 py-3 text-xs text-[#617286]"><Info className="h-4 w-4" />Land {Math.round(landArea)} m² · Footprint {Math.round(footprint)} m²</span>}</div>
      </section> : null}
      ...
      {step === 3 ? <div className="grid gap-5 xl:grid-cols-[1fr_0.38fr]">
        ...
        <aside className="space-y-4">
          ...
          <div className="grid gap-2"><button type="button" onClick={createPreliminaryBill} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#081B36] px-4 py-3 text-sm font-bold text-white"><FileSpreadsheet className="h-4 w-4" />Create preliminary cost plan</button><button type="button" onClick={useAsBudget} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#E9F8F1] px-4 py-3 text-sm font-bold text-[#087A50]"><WalletCards className="h-4 w-4" />Use as project budget</button><button type="button" onClick={saveScope} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#CAD5E0] px-4 py-3 text-sm font-bold text-[#617286]"><Save className="h-4 w-4" />Save project information</button></div>
