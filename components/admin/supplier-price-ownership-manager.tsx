...
export default function SupplierPriceOwnershipManager() {
  ...
  const [reviewReason, setReviewReason] = useState("Please confirm or update this price.");
  ...
  const beginReview = (offer: Offer) => {
    setReviewId(offer.id);
    setEditId(null);
    setIncludeProposal(false);
    setReviewReason("Please confirm or update this price.");
  ...
  const requestSellerReview = async (offer: Offer, reason: string) => {
  ...
  const submitProposal = async (offer: Offer, reason: string) => {
    ...
    closeReview();
    setMessage("Review request and exact proposal sent. The seller must see and authorise those exact values before Charismak can apply them.");
    await load(); setBusyId(null);
    const raw = offer.whatsapp || offer.phone;
    if (raw) {
      const digits = raw.replace(/\D/g, "").replace(/^0/, "234");
      const text = [`Hello ${offer.supplier_name},`, `Charismak has requested a review and proposes this exact change for ${offer.product_name}:`, `${money(Number(offer.unit_price))} / ${offer.quoted_unit} → ${money(Number(patch.unit_price))} / ${patch.quoted_unit}`, patch.specification !== offer.specification ? `Specification: ${offer.specification || "—"} → ${patch.specification || "—"}` : "", patch.brand !== offer.brand ? `Brand: ${offer.brand || "—"} → ${patch.brand || "—"}` : "", reason.trim() ? `Reason: ${reason.trim()}` : "", "Please sign in to review the proposal. You may update the price yourself or authorise Charismak to apply exactly the values shown.", "https://www.charismakproject.com/supplier-prices"].filter(Boolean).join("\n");
      window.open(`https://wa.me/${digits}?text=${encodeURIComponent(text)}`, "_blank", "noopener,noreferrer");
    }
  };

  const submitReviewRequest = async (offer: Offer) => {
    if (!reviewReason.trim()) { setError("Enter a reason or short note for the seller."); return; }
    if (includeProposal) await submitProposal(offer, reviewReason);
    else await requestSellerReview(offer, reviewReason);
  };

  const beginUnclaimedEdit = (offer: Offer) => { if (offer.supplier_id) return; setEditId(offer.id); setReviewId(null); setDraft(toDraft(offer)); setError(""); setMessage(""); };
  ...
  const removeOffer = async (offer: Offer) => {
    ...
    setMessage("Price removed from the live marketplace. Audit history retained."); setReviewId(null); setEditId(null); setDraft(null); await load(); setBusyId(null);
  };
  ...
  return <div className="mx-auto max-w-7xl px-4 py-8 md:px-8 md:py-12">
    <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#A82B05]">Supplier administration</p><h1 className="mt-2 text-3xl font-black text-[#071E33] md:text-4xl">Approved Supplier Prices</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-[#617286]">For a supplier-owned price, use Request Price Review. You can ask the seller to confirm or update it without suggesting a value, or include an exact proposal for the seller to review and authorise.</p></div><button type="button" disabled={loading} onClick={() => void load()} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#DCE4EC] bg-white px-4 text-xs font-black text-[#0D3B66]"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />Refresh</button></div>
    ...
    {loading ? <div className="mt-8 flex items-center gap-2 text-sm text-[#617286]"><Loader2 className="h-5 w-5 animate-spin" />Loading prices…</div> : <div className="mt-6 space-y-4">{visible.map((offer) => {
      ...
      return <article key={offer.id} className="overflow-hidden rounded-2xl border border-[#DCE4EC] bg-white shadow-[0_8px_30px_rgba(7,30,51,0.04)]">
        ...
        {reviewId === offer.id && draft ? <ReviewRequestPanel offer={offer} reason={reviewReason} setReason={setReviewReason} includeProposal={includeProposal} setIncludeProposal={setIncludeProposal} draft={draft} setDraft={setDraft} busy={busyId === offer.id} onSend={() => void submitReviewRequest(offer)} onCancel={closeReview} /> : null}
...
function ReviewRequestPanel({ offer, reason, setReason, includeProposal, setIncludeProposal, draft, setDraft, busy, onSend, onCancel }: { offer: Offer; reason: string; setReason: (value: string) => void; includeProposal: boolean; setIncludeProposal: (value: boolean) => void; draft: Draft; setDraft: (draft: Draft) => void; busy: boolean; onSend: () => void; onCancel: () => void }) {
  return <div className="border-t border-[#DCE4EC] bg-[#F8FAFC] p-5 md:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.12em] text-[#A82B05]">Request Price Review</p><h3 className="mt-1 text-lg font-black text-[#071E33]">{offer.product_name} · {offer.supplier_name}</h3><p className="mt-1 text-xs text-[#617286]">Current: {money(Number(offer.unit_price))} / {offer.quoted_unit}. Choose whether to request confirmation only or include your exact proposal.</p></div><button type="button" onClick={onCancel} className="grid h-9 w-9 place-items-center rounded-lg border border-[#DCE4EC] bg-white"><X className="h-4 w-4" /></button></div>
    <label className="mt-5 block"><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.1em] text-[#617286]">Reason / note to seller</span><textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={3} className="w-full rounded-xl border border-[#D5DFE8] bg-white px-3 py-3 text-sm text-[#071E33] outline-none focus:border-[#0D3B66]" placeholder="Why should the seller review this price?" /></label>
    <div className="mt-5 grid gap-3 md:grid-cols-2"><button type="button" onClick={() => setIncludeProposal(false)} className={`rounded-xl border p-4 text-left ${!includeProposal ? "border-[#0D3B66] bg-[#EEF5FB]" : "border-[#DCE4EC] bg-white"}`}><span className="block text-sm font-black text-[#071E33]">No proposal</span><span className="mt-1 block text-xs leading-5 text-[#617286]">Ask the seller to confirm the current price or update it themselves.</span></button><button type="button" onClick={() => setIncludeProposal(true)} className={`rounded-xl border p-4 text-left ${includeProposal ? "border-[#0D3B66] bg-[#EEF5FB]" : "border-[#DCE4EC] bg-white"}`}><span className="block text-sm font-black text-[#071E33]">Include my proposal</span><span className="mt-1 block text-xs leading-5 text-[#617286]">Suggest exact new values for the seller to review. Charismak still cannot apply them without seller authorisation.</span></button></div>
    {includeProposal ? <div className="mt-5"><p className="mb-3 text-xs font-black text-[#071E33]">Proposed values</p><CommercialFields draft={draft} setDraft={setDraft} /></div> : <div className="mt-5 rounded-xl border border-[#C9D8E7] bg-white p-4 text-xs leading-5 text-[#526579]"><strong className="text-[#071E33]">No price is being proposed.</strong> The seller will simply be asked to confirm the current listing or submit their own update.</div>}
    <div className="mt-5 flex flex-wrap gap-2"><button type="button" disabled={busy} onClick={onSend} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#197447] px-5 text-xs font-black text-white disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserCheck className="h-4 w-4" />}{includeProposal ? "Send review + proposal" : "Send review request"}</button><button type="button" onClick={onCancel} className="min-h-11 rounded-xl border border-[#DCE4EC] bg-white px-5 text-xs font-black text-[#526579]">Cancel</button></div>
  </div>;
}
...
function EditPanel({ title, note, draft, setDraft, busy, onSave, saveLabel, onCancel }: { title: string; note: string; draft: Draft; setDraft: (draft: Draft) => void; busy: boolean; onSave: () => void; saveLabel: string; onCancel: () => void }) {
  return <div className="border-t border-[#DCE4EC] bg-[#F8FAFC] p-5 md:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.12em] text-[#A82B05]">{title}</p><p className="mt-1 text-xs text-[#617286]">{note}</p></div><button type="button" onClick={onCancel} className="grid h-9 w-9 place-items-center rounded-lg border border-[#DCE4EC] bg-white"><X className="h-4 w-4" /></button></div><div className="mt-5"><CommercialFields draft={draft} setDraft={setDraft} /></div><div className="mt-5 flex flex-wrap gap-2"><button type="button" disabled={busy} onClick={onSave} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#197447] px-5 text-xs font-black text-white disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{saveLabel}</button><button type="button" onClick={onCancel} className="min-h-11 rounded-xl border border-[#DCE4EC] bg-white px-5 text-xs font-black text-[#526579]">Cancel</button></div></div>;
}
