...
export default function Sidebar({ activePage, onSelectPage, isAdmin = false }: SidebarProps) {
  return (
    <aside className="hidden w-[300px] shrink-0 flex-col bg-[#071E33] px-6 py-8 text-white lg:flex">
      ...
      <nav className="flex flex-1 flex-col gap-2">
        {[...navItems, ...(isAdmin ? [{ key: "insights" as PageKey, label: "Beta Insights" }] : [])].map((item) => {
